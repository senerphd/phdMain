...
\begin{equation}
    \boxed{u = \hat{L}^T z + \hat{M}\,r + \hat{d}}
...
\begin{itemize}
    ...
    \item $\hat{d} \in \mathbb{R}$: \textbf{Adaptif bias} terimi. Sabit dış kuvvetler veya bilinmeyen DC ofsetleri telafi eder.
\end{itemize}
...
\begin{itemize}
    ...
    \item $\hat{d}$: Yan rüzgâra karşı sürekli hafif direksiyon kırma (sabit bozucuyu telafi eder)
\end{itemize}
\end{kavram}
...
İdeal dünyada, bilinmeyen parametrelerin tam değerleri ($L^*, M^*, d^*$) şu koşulları sağlar:
\begin{align}
    A + B\,{L^*}^T &= A_m \label{eq:match_L}\\
    B\,M^* &= B_m \label{eq:match_M}\\
    B\,d^* &= -B_d\,F_{ext} \label{eq:match_d}
\end{align}
...
\begin{equation}
    V = e^T P\,e + \frac{1}{\Gamma_L}\,\tilde{L}^T\tilde{L} + \frac{1}{\Gamma_M}\,\tilde{M}^2 + \frac{1}{\Gamma_d}\,\tilde{d}^2
    \label{eq:lyapunov}
\end{equation}
...
\begin{itemize}
    ...
    \item $\tilde{d} = \hat{d} - d^*$: Bias terimi parametre hatası
    \item $\Gamma_L, \Gamma_M, \Gamma_d > 0$: Adaptasyon hızı parametreleri
\end{itemize}
...
\begin{equation}
    \boxed{\dot{\hat{L}} = -\Gamma_L \cdot \underbrace{\left(e^T P\,B\right)}_{\text{skaler terim}} \cdot z \cdot \Lambda - \sigma_L \cdot \hat{L}}
    \label{eq:L_update}
\end{equation}

\subsubsection{İleri Besleme Kazancı ($\hat{M}$)}
\begin{equation}
    \boxed{\dot{\hat{M}} = -\Gamma_M \cdot \left(e^T P\,B\right) \cdot r - \sigma_M \cdot \hat{M}}
    \label{eq:M_update}
\end{equation}

\subsubsection{Adaptif Bias Terimi ($\hat{d}$)}
\begin{equation}
    \boxed{\dot{\hat{d}} = -\Gamma_d \cdot \left(e^T P\,B\right) - \sigma_d \cdot \hat{d}}
    \label{eq:d_update}
\end{equation}

Burada her kuralda ortak olan $e^T P\,B$ terimi, tek bir skaler sayıdır ve tüm adaptasyon mekanizmasının ``yakıtı''dır.
...
\subsection{Problem 2: Kazanç Şişmesi ve $\sigma$-Modifikasyonu}
\label{subsec:sigma}
...
\subsubsection{Çözüm: Sızıntı Terimi ($\sigma$-Modification / Leakage)}

Her adaptasyon kuralına, kazançları sıfıra doğru ``çeken'' bir sönümleme terimi eklenir:

\begin{equation}
    \dot{\hat{L}} = -\Gamma_L \cdot (e^T P B) \cdot z \cdot \Lambda \underbrace{- \sigma_L \cdot \hat{L}}_{\text{sızıntı terimi}}
\end{equation}

Fiziksel analoji: $\sigma_L$, kazancın bağlı olduğu bir \textbf{yay} gibidir. Kazanç ne kadar büyürse, yay onu o kadar güçlü geri çeker.
...
Bu çalışmada, simülasyon testleri sonucunda $\sigma_L = \sigma_M = \sigma_d = 1$ değeri, pürüzsüz kontrol sinyali ve yeterli adaptasyon kapasitesi arasındaki en iyi dengeyi sağlamıştır.
...
\subsection{Problem 4: Dış Kuvvet Bozucusu Reddi}
\label{subsec:disturbance}

Standart MRAC kontrol kuralı ($u = \hat{L}^T z + \hat{M} r$) yalnızca durumlara ($z$) ve referansa ($r$) bağlı terimler içerir. Sisteme sabit bir dış kuvvet ($F_{ext}$) uygulandığında, bunu telafi etmek için kontrol sinyalinde \textbf{sabit bir DC bileşen} gerekir.

\subsubsection{Neden $\hat{L}$ ve $\hat{M}$ Yetersiz Kalır?}
$\hat{L}^T z$ terimi durumlara çarpıldığı için, ancak durumlar sıfır olmadığında (yani hata varken) bir akım üretebilir. Sistem hedefe yaklaştığında durumlar küçülür ve telafi edici akım da kaybolur. MRAC, bu paradoksu çözmek için $\hat{L}$ kazançlarını şişirmeye çalışır, bu da chattering'e yol açar.

\subsubsection{Çözüm: Adaptif Bias Terimi ($\hat{d}$)}
Kontrol kuralına durumlardan ve referanstan bağımsız, sabit bir terim eklenir:
\begin{equation}
    u = \hat{L}^T z + \hat{M}\,r + \hat{d}
\end{equation}

$\hat{d}$'nin adaptasyon kuralı (\ref{eq:d_update}):
\begin{equation}
    \dot{\hat{d}} = -\Gamma_d \cdot (e^T P B) - \sigma_d \cdot \hat{d}
\end{equation}

$\hat{d}$, dış kuvveti karşılamak için gereken sabit akım değerine yakınsar. Bu sayede $\hat{L}$ kazançları nominal değerlerinde kalır ve chattering oluşmaz.

\begin{cozum}[Fiziksel Analoji]
Bir terazinin bir kefesine bilinmeyen bir ağırlık konulduğunu düşünün. $\hat{L}$ ve $\hat{M}$, terazinin kollarını ayarlayarak dengelemeye çalışır (ama kol uzunluğu sınırlıdır). $\hat{d}$ ise doğrudan diğer kefeye karşı ağırlık koyarak dengeyi sağlar.
\end{cozum}
...
$\Gamma_d$ & Yavaş bozucu telafisi & Hızlı telafi, gürültüye hassas \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Bias terimi olmadan:} $\hat{L}$ kazançları nominal değerlerinin 50 katına şişmiş, $u_{mA}$ sinyali $\pm 25$~mA arasında tam doyum chattering'i göstermiştir.

\textbf{Bias terimi ($\hat{d}$) ile:} $\hat{d}$, dış kuvveti karşılayan sabit akım değerine yakınsamış, $\hat{L}$ kazançları nominal değerlerinde kalmış, chattering tamamen ortadan kalkmıştır.
...
MRAC'ın gerçek gücünü görmek için, yalnızca çıkış ($z_1$) değil, adaptif kazançların ($\hat{L}, \hat{M}, \hat{d}$) zamanla değişimini incelemek kritik önem taşır.
...
Bias & $\Gamma_d$ & $50 \times 10^{-3}$ & Bozucu telafi hızı \\
\midrule
\multicolumn{4}{l}{\textbf{Sızıntı ($\sigma$-mod)}} \\
\midrule
Geri besleme freni & $\sigma_L$ & $1$ & Kazanç şişmesi önleme \\
İleri besleme freni & $\sigma_M$ & $1$ & $\hat{M}$ sınırlama \\
Bias freni & $\sigma_d$ & $1$ & $\hat{d}$ sınırlama \\
\midrule
...
$\hat{d}$ başlangıç & $\hat{d}(0)$ & $0$ & Bozucu olmadığı varsayımı \\
...
\begin{enumerate}
    ...
    \item \textbf{$\sigma$-Modifikasyonu Pratik Bir Zorunluluktur:} Saf MRAC, ölçüm gürültüsü ve modellenmemiş sürtünme karşısında kazanç şişmesine maruz kalır. $\sigma = 1$ değeri, bu çalışmadaki hidrolik sistem için pürüzsüz kontrol sinyali ve yeterli adaptasyon kapasitesi arasındaki optimum dengeyi sağlamıştır.

    \item \textbf{Dış Kuvvet Reddi İçin Bias Terimi Şarttır:} Standart MRAC yapısı ($u = \hat{L}^T z + \hat{M} r$), sabit dış kuvvetleri telafi edemez. Adaptif bias terimi ($\hat{d}$) eklenmesi, kazanç şişmesi ve chattering'i önlerken dış kuvveti başarıyla reddeder.
