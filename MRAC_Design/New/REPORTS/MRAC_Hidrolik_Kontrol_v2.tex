...
\begin{equation}
    \boxed{\dot{\hat{L}} = -\Gamma_L \cdot \underbrace{\left(e^T P\,B\right)}_{\text{skaler terim}} \cdot z \cdot \Lambda - \sigma_L \cdot \|e\| \cdot \hat{L}}
    \label{eq:L_update}
\end{equation}

\subsubsection{İleri Besleme Kazancı ($\hat{M}$)}
\begin{equation}
    \boxed{\dot{\hat{M}} = -\Gamma_M \cdot \left(e^T P\,B\right) \cdot r - \sigma_M \cdot \|e\| \cdot \hat{M}}
    \label{eq:M_update}
\end{equation}
...
\subsubsection{Çözüm: $e$-Modifikasyonu (Hata Orantılı Sızıntı)}
...
\begin{equation}
    \dot{\hat{L}} = -\Gamma_L \cdot (e^T P B) \cdot z \cdot \Lambda \underbrace{- \sigma_L \cdot \|e\| \cdot \hat{L}}_{\text{e-modifikasyon terimi}}
\end{equation}

Fiziksel analoji: $\sigma_L$, kazancın bağlı olduğu bir yay gibidir, ancak bu yayın sertliği hata büyükken (yay devrede) yüksek, hata sıfıra yaklaştığında (yay gevşek) çok düşüktür. Standart $\sigma$-modifikasyonuna kıyasla $e$-modifikasyonu, kalıcı hatayı (steady-state error) daha az bozar.
