...
\textbf{Adım 1:} İzleme hatasını (tracking error) $e = z_m - z$ olarak tanımlayalım ve zamana göre türevini alalım:
\begin{equation}
    \dot{e} = \dot{z}_m - \dot{z} = (A_m z_m + B_m r) - (A z + B u)
\end{equation}

\textbf{Adım 2:} Kontrol kuralını (Denklem \ref{eq:control}) $\dot{e}$ içerisine yerleştirelim:
\begin{equation}
    \dot{e} = A_m z_m + B_m r - A z - B (\hat{L}^T z + \hat{M} r)
\end{equation}
...
\begin{align}
    \dot{e} &= A_m z_m + B M^* r - (A_m - B (L^*)^T) z - B \hat{L}^T z - B \hat{M} r \nonumber \\
    \dot{e} &= A_m (z_m - z) - B (\hat{L}^T - (L^*)^T) z - B (\hat{M} - M^*) r
\end{align}
